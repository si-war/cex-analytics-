...
```
.
├── CEX_Analytics_corrige.ipynb      ← Main notebook (run this one)
├── app/
│   ├── app.py                       ← Streamlit web application
│   └── preprocessing.py             ← Feature engineering & prediction logic
├── output/
│   ├── features_v1.csv              ← Engineered features (generated by notebook)
│   ├── processed_data_v1.csv        ← Scaled features + target (generated by notebook)
│   ├── clean_data_v1.csv            ← Cleaned raw data (generated by notebook)
│   └── final_model.joblib           ← Trained stacking ensemble (generated by notebook)
├── dataset/                         ← CE-PUMD data files (you provide these)
│   └── intrvw24 (1)/
│       └── intrvw24/
│           └── data needed/
│               ├── fmli*.csv        ← Family-level interview data (5 quarters)
│               └── memi*.csv        ← Member-level education data (5 quarters)
├── requirements.txt
└── README.md
```
